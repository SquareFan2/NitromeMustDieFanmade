...
public class SmallApeCopy : MonoBehaviour
{
    ...
    [Header("死亡效果")]
    public GameObject destroy1; //死亡效果1预制件
    public GameObject coinGenerator; //金币生成器
    private Color cl = new Color(1, 0.925f, 0, 1); //死亡液体颜色
    private int dripCount = 6; //死亡时溅出的水滴数量（浮动线）
    private int coinCount = 5; //死亡时掉落金币数量
    ...
    void Movement()
    {
        ...
        Vector2 offset = new Vector2(0.46875f, 0f);
        ...
        float wallDir = 0.5f; //墙壁检测射线的长度
        RaycastHit2D leftCheckWall = Physics2D.Raycast(pos + offsetWall, Vector2.left, wallDir, ground); //左射线
        RaycastHit2D rightCheckWall = Physics2D.Raycast(pos + offsetWall, Vector2.right, wallDir, ground); //右射线
        Debug.DrawRay(pos + offsetWall, Vector2.left * wallDir, Color.red, 0.2f);
        Debug.DrawRay(pos + offsetWall, Vector2.right * wallDir, Color.red, 0.2f);
        if (leftCheckWall)
        {
            face = 1f;
            faceDir = 1f;
